...
document.getElementById("base").style.height =
    document.querySelector(".rod").offsetWidth + "px"
...
document.addEventListener("drop", (event) => {
    ...
    if (event.target.className == "rodContainer" || event.target.className == "rod") {
        let rod = undefined
        event.target.className == "rodContainer"
            ? (rod = event.target)
            : (rod = event.target.parentElement)
...
document.addEventListener("click", (event) => {
    if (event.target.id == "giveUp") {
        ...
        document
            ...
            .forEach((ring) => (ring.style.width = ring.offsetWidth + "px"))
        autoMoves = []
    ...
    if (event.target.classList.contains("next")) {
        document.getElementById("giveUp").outerHTML =
            '<button title="(get help from the computer)" class="button" id="giveUp" onclick="this.disabled = true">I Give Up!</button>'
        if (
            parseInt(event.target.id.substring(event.target.id.length - 1)) < 6
        ) {
            nRings++
            initializeGame(nRings)
            ...
            document
                .getElementById("level" + (nRings + 1))
                .classList.remove("noAccess")
            document
                .getElementById("level" + (nRings + 1))
                .classList.add("next")
            document.getElementById("win").style.display = "none"
            document.getElementById("modalContainer").style.visibility =
                "hidden"
        } else if (
            parseInt(event.target.id.substring(event.target.id.length - 1)) == 6
        ) {
...
function initializeGame(nRings) {
    ...
    yLevels = []
    const rodWidth = parseInt(
        window.getComputedStyle(document.getElementById("rodContainer1")).width
    )
    for (let i = 0; i < nRings; i++) {
        ...
        if (nRings < 5) {
            ...
            ring.style.height = `calc(80% / ${nRings})`
        }
        ring.style.width =
            ((nRings * 2 - 1 - 2 * i) / (nRings * 2 - 1)) * 100 + "%"
        ring.draggable = true
...
function autoAddRing(start, end, i) {
    ...
    if (nRings < 11) {
        ...
        styleSheet.insertRule(
            ...
            transform: translate(${
                endRod.getBoundingClientRect().left -
                startRod.getBoundingClientRect().left
            }px, ${yLevels[endRod.childNodes.length - 1] - ringRect.top}px);
...
function addRing(ring, rings, rod, event) {
    ...
    if (rings.length == 1 || rings[1].offsetWidth > ring.offsetWidth) {
        ...
        if (yLevels[rod.childNodes.length - 1] - event.clientY - offsetY - offH > 0) {
            ...
            `.drop {
            position: absolute;
            left: ${(rodRect.left + rodRect.right) / 2 - width / 2 - offW}px;
